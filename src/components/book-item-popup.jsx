...
class BookItemPopup extends React.Component {
    ...
    render() {
        const { getRating, clicked, getImage, clickPopup, getAuthor, getTitle, getDescription, getLink } = this.props;
        const activePopup = clicked ? 'book-item-popup_active' : '';
        const activeOverlay = clicked ? 'book-item-popup__overlay_active' : '';
        return (
            <React.Fragment>
                <CSSTransition classNames="popup" unmountOnExit in={this.state.clicked} timeout={300}>
                    <div className={`book-item-popup ${activePopup}`} style={{
